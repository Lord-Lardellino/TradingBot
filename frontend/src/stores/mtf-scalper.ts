...
export const useMtfScalperStore = defineStore('mtf-scalper', () => {
  ...
  async function loadAnalytics() {
    loading.value = true
    try {
      const { data } = await axios.get('/api/mtf-scalper/analytics')
  ...
  async function updateConfig(patch: Record<string, any>) {
    const { data } = await axios.patch('/api/mtf-scalper/config', patch)
    if (analytics.value) analytics.value.config = data
    return data
  }

  async function resetSim() {
    await axios.post('/api/mtf-scalper/reset')
    await loadAnalytics()
  }
  ...
  function connect() {
    ...
    sock.on('mtf-scalper:signal', (sig: any) => {
      signals.value.unshift(sig)
      if (signals.value.length > 250) signals.value.pop()
    })

    sock.on('mtf-scalper:trade', () => { loadAnalytics() })

    sock.on('mtf-scalper:status', (s: any) => {
      if (analytics.value) analytics.value.scannerStatus = s
    })

    sock.on('mtf-scalper:positions', (pos: any[]) => {
