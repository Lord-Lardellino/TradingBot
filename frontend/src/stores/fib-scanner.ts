...
export const useFibScannerStore = defineStore('fib-scanner', () => {
  ...
  async function loadAnalytics() {
    loading.value = true
    try {
      const { data } = await axios.get('/api/fib-scanner/analytics')
  ...
  async function updateConfig(patch: Record<string, any>) {
    const { data } = await axios.patch('/api/fib-scanner/config', patch)
    if (analytics.value) analytics.value.config = data
    return data
  }

  async function resetSim() {
    await axios.post('/api/fib-scanner/reset')
    await loadAnalytics()
  }
  ...
  function connect() {
    ...
    sock.on('fib:trade', () => { loadAnalytics() })

    sock.on('fib:status', (s: any) => {
      if (analytics.value) analytics.value.scannerStatus = s
    })

    sock.on('fib:positions', (pos: any[]) => {
