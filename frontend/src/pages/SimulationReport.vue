      ...
      <div class="stat-card">
        ...
        <div class="space-y-2 max-h-[640px] overflow-y-auto -mx-1 px-1">
          ...
          <div
            ...
            class="rounded-xl border px-4 py-3 flex items-center gap-5 transition-colors group"
          >
            <!-- ① Simbolo + meta -->
  ...
  setup(props) {
    ...
    onMounted(() => {
      ...
      const data = (props.curve as any[]).map((p, i) => ({
        time: Math.floor(new Date(p.date).getTime() / 1000) + i,
        value: p.capital,
      }))
      series.setData(data)
...
function statusClass(s: string) {
  if (s === 'open') return 'text-yellow-400 font-mono text-xs'
  if (s === 'tp1' || s === 'tp2') return 'text-profit font-mono text-xs font-bold'
  if (s === 'sl') return 'text-loss font-mono text-xs font-bold'
  return 'text-gray-400 font-mono text-xs'
}

function statusLabel(s: string) {
  return { open: '● Aperto', tp1: '✅ TP1', tp2: '✅ TP2', sl: '❌ SL', manual: '⬜ Manuale' }[s] ?? s
}
