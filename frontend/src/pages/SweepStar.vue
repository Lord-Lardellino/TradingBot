<template>
  <div class="p-3 sm:p-6 space-y-4">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
      <div>
        <h1 class="text-xl font-bold text-white flex items-center gap-2">
          <span class="inline-block w-2.5 h-2.5 rounded-full bg-violet-400 animate-pulse" />
          Sweep Star Scanner
          <span class="text-xs font-normal text-gray-500">Liquidity sweep + reversal candle · Top 100 coppie</span>
        </h1>
        <p class="text-xs text-gray-500 mt-0.5">
          Scan continuo ·
          <span :class="sweepStatus?.isScanning ? 'text-violet-400 animate-pulse' : 'text-gray-600'">
            {{ sweepStatus?.isScanning ? '⚡ Scanning...' : '● Idle' }}
          </span>
          <span v-if="sweepStatus?.lastScanAt" class="ml-2">· {{ timeAgo(sweepStatus.lastScanAt) }}</span>
          <span v-if="sweepStatus?.scannedPairs" class="ml-2 text-gray-600">· {{ sweepStatus.scannedPairs }} coppie</span>
        </p>
      </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
      ...
      <div class="stat-card text-center">
        <div class="text-3xl font-bold font-mono text-violet-400">{{ analytics?.openTrades ?? 0 }}</div>
    ...
    <div class="stat-card">
      <div class="flex flex-wrap gap-2 items-center">
        ...
        <span class="text-xs text-gray-600 font-mono ml-2">
          ...
          · sweep ≥{{ analytics?.config?.minSweepWickRange ?? 0.5 }}
      ...
      <div v-if="showConfig" class="mt-3 pt-3 border-t border-white/5 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
        ...
        <div class="flex flex-col gap-1">
          <label class="text-xs text-gray-500">Min Sweep Wick Range</label>
          <InputNumber v-model="cfg.minSweepWickRange" :min="0" :step="0.01" :maxFractionDigits="2" @update:modelValue="v => store.updateConfig({ minSweepWickRange: v })" />
        </div>
        <div class="flex flex-col gap-1">
          <label class="text-xs text-gray-500">Min Confirm Body ATR</label>
          <InputNumber v-model="cfg.minConfirmBodyAtr" :min="0" :step="0.01" :maxFractionDigits="2" @update:modelValue="v => store.updateConfig({ minConfirmBodyAtr: v })" />
        </div>
        <div class="flex flex-col gap-1">
        ...
        <div class="flex flex-col gap-1">
          <label class="text-xs text-gray-500">Liquidity Lookback</label>
          <InputNumber v-model="cfg.liquidityLookback" :min="1" :step="1" :maxFractionDigits="0" @update:modelValue="v => store.updateConfig({ liquidityLookback: v })" />
        </div>
    ...
    <!-- Debug -->
    <div v-if="sweepStatus?.debug && Object.keys(sweepStatus.debug).length" class="stat-card">
      <div class="text-xs text-gray-500 mb-2">Debug filtri</div>
      <div class="flex flex-wrap gap-2">
        <span v-for="(v, k) in sweepStatus.debug" :key="k"
    ...
    <div class="stat-card space-y-2">
      ...
      <div v-else class="space-y-1.5">
        <button v-for="sig in store.signals.slice(0, 30)" :key="sig.id"
          ...
          <span class="text-xs font-mono px-2 py-0.5 rounded-full bg-violet-500/15 text-violet-300">
            sweep {{ sig.sweepWickRange?.toFixed(2) }}
          </span>
          ...
          <i class="pi pi-chart-line text-violet-500 text-sm" />
        </button>
      </div>
    </div>

    <!-- Trade aperti -->
    <div v-if="store.openTrades.length" class="stat-card space-y-2">
      <h3 class="text-base font-semibold text-white">Trade simulati aperti</h3>
      <button v-for="t in store.openTrades" :key="t.id" type="button"
        class="w-full text-left flex items-center gap-3 px-4 py-3 rounded-xl bg-surface-200 border border-white/5 hover:bg-black/35 hover:border-violet-400/25 transition flex-wrap"
    ...
    <div class="stat-card space-y-2">
      ...
      <div v-if="!store.closedTrades.length" class="text-sm text-gray-600 py-6 text-center">Nessun trade chiuso ancora</div>
      <div v-else class="overflow-x-auto">
        <table class="w-full text-sm font-mono">
          ...
          <tbody>
            <tr v-for="t in store.closedTrades.slice(0, 100)" :key="t.id"
              ...
              <td class="py-2.5 px-3 text-right text-gray-500">{{ t.closedAt ? timeAgo(t.closedAt) : '—' }}<i class="pi pi-chart-line text-violet-500 text-xs ml-2" /></td>
            </tr>
          </tbody>
        </table>
      </div>
        ...
        <div v-if="selected" class="fixed inset-0 z-50 flex items-start justify-center p-4 pt-8 overflow-y-auto" @click.self="closeDialog">
          ...
          <div class="relative z-10 w-full max-w-5xl rounded-2xl bg-[#0d0f14] border border-white/10 shadow-2xl mb-8">
            <div class="flex items-center gap-3 px-6 py-4 border-b border-white/8 sticky top-0 z-10 bg-[#0d0f14] rounded-t-2xl">
              ...
              <span class="text-xs font-mono px-2 py-1 rounded bg-violet-500/15 text-violet-300">sweep {{ selected.sweepWickRange?.toFixed(2) }}</span>
            ...
            <div class="p-6 space-y-5">
              ...
              <div class="grid grid-cols-3 gap-4">
                ...
                <div class="rounded-xl bg-black/30 border border-white/5 p-4 space-y-3">
                  <div class="text-xs text-gray-500 uppercase tracking-wider font-semibold">Sweep</div>
                  <div><div class="text-xs text-gray-500 mb-0.5">Sweep Wick Range</div><div class="text-2xl font-black text-violet-400 font-mono">{{ selected.sweepWickRange?.toFixed(3) }}</div></div>
                  <div><div class="text-xs text-gray-500 mb-0.5">Body Range Ratio</div><div class="text-xl font-black text-white font-mono">{{ selected.bodyRangeRatio?.toFixed(3) }}</div></div>
                  <div><div class="text-xs text-gray-500 mb-0.5">Volume Ratio</div><div class="text-xl font-black text-white font-mono">{{ selected.volumeRatio?.toFixed(3) }}x</div></div>
                </div>
                <div class="rounded-xl bg-black/30 border border-white/5 p-4 space-y-3">
                  ...
                  <div class="text-center py-2 border-t border-white/5">
                    <div class="text-xs text-gray-500 mb-1">Sweep wick range</div>
                    <div class="text-3xl font-black font-mono text-violet-400">{{ selected.sweepWickRange?.toFixed(3) }}</div>
                  </div>
                </div>
              </div>
...
import { useSweepStarStore } from '@/stores/sweep-star'

const store      = useSweepStarStore()
const analytics  = computed(() => store.analytics)
// questo store espone status come ref separata
const sweepStatus = computed(() => store.status)
...
function normalizeDialogItem(item: any) {
  ...
  const takeProfit = Number(item.takeProfit ?? item.takeProfit1 ?? 0)
  const reasons = item.reasons ?? [`Trade ${item.direction}`, `Entry ${item.entry}`, `SL ${stopLoss}`, `TP ${takeProfit}`]
  return {
    ...item, stopLoss, takeProfit,
    ...
    sweepWickRange: Number(item.sweepWickRange ?? 0),
    bodyRangeRatio: Number(item.bodyRangeRatio ?? 0),
...
function isLiveChart(sig: any) { return !sig.closedAt && !['tp', 'tp1', 'sl', 'manual'].includes(String(sig.status ?? '').toLowerCase()) }
function chartCandlesUrl(sig: any) {
  ...
  return from ? `/api/sweep-star/candles/${sym}?limit=120&from=${from}${live}` : `/api/sweep-star/candles/${sym}?limit=120${live}`
}
...
function timeAgo(ts: string | number | null) { if (!ts) return '—'; const ms = typeof ts === 'number' ? ts : new Date(ts).getTime(); const s = Math.floor((Date.now() - ms) / 1000); if (s < 60) return `${s}s fa`; const m = Math.floor(s / 60); if (m < 60) return `${m}m fa`; return `${Math.floor(m / 60)}h fa` }
