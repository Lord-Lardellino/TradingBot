<template>
  <div class="p-3 sm:p-6 space-y-4">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
      <div>
        <h1 class="text-xl font-bold text-white flex items-center gap-2">
          <span class="inline-block w-2.5 h-2.5 rounded-full bg-rose-400 animate-pulse" />
          Impulse 100 Scanner
          <span class="text-xs font-normal text-gray-500">Continuation pullback + failed move reversal · Top 100 coppie</span>
        </h1>
        <p class="text-xs text-gray-500 mt-0.5">
          Scan continuo ·
          <span :class="impulseStatus?.isScanning ? 'text-rose-400 animate-pulse' : 'text-gray-600'">
            {{ impulseStatus?.isScanning ? '⚡ Scanning...' : '● Idle' }}
          </span>
          <span v-if="impulseStatus?.lastScanAt" class="ml-2">· {{ timeAgo(impulseStatus.lastScanAt) }}</span>
          <span v-if="impulseStatus?.scannedPairs" class="ml-2 text-gray-600">· {{ impulseStatus.scannedPairs }} coppie</span>
        </p>
      </div>
      <Button size="small" icon="pi pi-refresh" label="Aggiorna" severity="secondary" :loading="store.loading" @click="store.fetchAll()" />
    </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
      ...
      <div class="stat-card text-center">
        <div class="text-3xl font-bold font-mono text-rose-400">{{ analytics?.openTrades ?? 0 }}</div>
    ...
    <div class="stat-card">
      <div class="flex flex-wrap gap-2 items-center">
        ...
        <button @click="toggleEnabled"
          :class="['text-sm px-3 py-1 rounded-full font-medium transition', analytics?.config?.enabled ? 'bg-rose-500/20 text-rose-400' : 'bg-surface-200 text-gray-500']">
        ...
        <span class="text-xs text-gray-600 font-mono ml-2">
          RR {{ analytics?.config?.tpRr ?? 2 }} · risk {{ analytics?.config?.riskUsdt ?? 1 }}$
      ...
      <div v-if="showConfig" class="mt-3 pt-3 border-t border-white/5 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
        ...
        <div class="flex flex-col gap-1">
          <label class="text-xs text-gray-500">TP RR</label>
          <InputNumber v-model="cfg.tpRr" :min="0.1" :step="0.1" :maxFractionDigits="2" @update:modelValue="v => store.updateConfig({ tpRr: v })" />
        </div>
        <div class="flex flex-col gap-1">
          <label class="text-xs text-gray-500">Min Body Range Ratio</label>
          <InputNumber v-model="cfg.minBodyRangeRatio" :min="0" :step="0.01" :maxFractionDigits="2" @update:modelValue="v => store.updateConfig({ minBodyRangeRatio: v })" />
        </div>
        <div class="flex flex-col gap-1">
          <label class="text-xs text-gray-500">Max Close Wick Range</label>
          <InputNumber v-model="cfg.maxCloseWickRange" :min="0" :step="0.01" :maxFractionDigits="2" @update:modelValue="v => store.updateConfig({ maxCloseWickRange: v })" />
        </div>
        <div class="flex flex-col gap-1">
          <label class="text-xs text-gray-500">Max Opposite Wick</label>
          <InputNumber v-model="cfg.maxOppositeWickRange" :min="0" :step="0.01" :maxFractionDigits="2" @update:modelValue="v => store.updateConfig({ maxOppositeWickRange: v })" />
        </div>
        <div class="flex flex-col gap-1">
          <label class="text-xs text-gray-500">Min Range ATR</label>
          <InputNumber v-model="cfg.minRangeAtr" :min="0" :step="0.01" :maxFractionDigits="2" @update:modelValue="v => store.updateConfig({ minRangeAtr: v })" />
        </div>
        <div class="flex flex-col gap-1">
          <label class="text-xs text-gray-500">Max Range ATR</label>
          <InputNumber v-model="cfg.maxRangeAtr" :min="0" :step="0.01" :maxFractionDigits="2" @update:modelValue="v => store.updateConfig({ maxRangeAtr: v })" />
        </div>
        <div class="flex flex-col gap-1">
        ...
        <div class="flex flex-col gap-1">
          <label class="text-xs text-gray-500">Max Spread %</label>
          <InputNumber v-model="cfg.maxSpreadPct" :min="0" :step="0.01" :maxFractionDigits="2" @update:modelValue="v => store.updateConfig({ maxSpreadPct: v })" />
        </div>
        <div class="flex flex-col gap-1">
        ...
        <div class="flex flex-col gap-1">
          <label class="text-xs text-gray-500">Max Segnali/Scan</label>
          <InputNumber v-model="cfg.maxSignalsPerScan" :min="1" :step="1" :maxFractionDigits="0" @update:modelValue="v => store.updateConfig({ maxSignalsPerScan: v })" />
        </div>
        <div class="flex flex-col gap-1">
        ...
        <div class="flex flex-col gap-1">
          <label class="text-xs text-gray-500">Top Pairs</label>
          <InputNumber v-model="cfg.topPairs" :min="1" :step="1" :maxFractionDigits="0" @update:modelValue="v => store.updateConfig({ topPairs: v })" />
        </div>
        <div class="flex flex-col gap-1">
          <label class="text-xs text-gray-500">Break Lookback</label>
          <InputNumber v-model="cfg.breakLookback" :min="1" :step="1" :maxFractionDigits="0" @update:modelValue="v => store.updateConfig({ breakLookback: v })" />
        </div>
        <div class="flex flex-col gap-1">
        ...
        <div class="flex flex-col gap-1">
          <label class="text-xs text-gray-500">Min Volume 24h</label>
          <InputNumber v-model="cfg.minVolume24h" :min="0" :step="1" :maxFractionDigits="0" @update:modelValue="v => store.updateConfig({ minVolume24h: v })" />
        </div>
        ...
        <div class="flex items-center justify-between gap-2 bg-black/20 rounded-lg px-3 py-2">
          ...
          <ToggleSwitch v-model="(cfg as any).autoEnter" @update:modelValue="v => store.updateConfig({ autoEnter: v })" />
        </div>
        <div class="flex items-center justify-between gap-2 bg-black/20 rounded-lg px-3 py-2">
          <span class="text-xs text-gray-400">Live</span>
          <ToggleSwitch v-model="(cfg as any).liveEnabled" @update:modelValue="v => store.updateConfig({ liveEnabled: v })" />
        </div>
      </div>
    ...
    <!-- Debug -->
    <div v-if="impulseStatus?.debug && Object.keys(impulseStatus.debug).length" class="stat-card">
      <div class="text-xs text-gray-500 mb-2">Debug filtri</div>
      <div class="flex flex-wrap gap-2">
        <span v-for="(v, k) in impulseStatus.debug" :key="k"
    ...
    <div class="stat-card space-y-2">
      ...
      <div v-if="!store.signals.length" class="text-sm text-gray-600 py-6 text-center">Nessun segnale — scan continuo</div>
      <div v-else class="space-y-1.5">
        <button v-for="sig in store.signals.slice(0, 30)" :key="sig.id"
          ...
          <span class="text-xs font-mono px-2 py-0.5 rounded-full bg-rose-500/15 text-rose-300">
            {{ sig.patternName?.replace('_LONG','').replace('_SHORT','').replace(/_/g,' ') }}
          </span>
          ...
          <i class="pi pi-chart-line text-rose-500 text-sm" />
        </button>
      </div>
    </div>
    ...
    <div v-if="openTrades.length" class="stat-card space-y-2">
      ...
      <button v-for="t in openTrades" :key="t.id" type="button"
        class="w-full text-left flex items-center gap-3 px-4 py-3 rounded-xl bg-surface-200 border border-white/5 hover:bg-black/35 hover:border-rose-400/25 transition flex-wrap"
        ...
        <span class="text-xs font-mono text-rose-300 px-2 py-0.5 rounded-full bg-rose-500/10">
          {{ t.patternName?.replace('_LONG','').replace('_SHORT','').replace(/_/g,' ') }}
        </span>
        <div class="flex gap-4 text-sm font-mono">
          ...
          <span>TP <span class="text-green-400 font-bold">{{ t.takeProfit1 }}</span></span>
        </div>
        ...
        <table class="w-full text-sm font-mono">
          <thead>
            <tr class="text-xs text-gray-500 uppercase tracking-wider border-b border-white/5">
              ...
              <th class="text-right py-2 px-3">Pattern</th>
          ...
          <tbody>
            <tr v-for="t in closedTrades.slice(0, 100)" :key="t.id"
              ...
              <td class="py-2.5 px-3 text-right text-rose-400">{{ t.patternName?.replace('_LONG','').replace('_SHORT','').replace(/_/g,' ') }}</td>
              <td class="py-2.5 px-3 text-right text-white">{{ Number((t as any).volumeRatio ?? 0).toFixed(2) }}x</td>
              <td class="py-2.5 px-3 text-right text-gray-400">{{ t.score }}pt</td>
              <td class="py-2.5 px-3 text-right text-yellow-400">{{ Number((t as any).slPct ?? 0).toFixed(2) }}%</td>
              <td class="py-2.5 px-3 text-right font-bold text-base" :class="pnlColor(t.pnl)">{{ t.pnl != null ? (t.pnl >= 0 ? '+' : '') + t.pnl.toFixed(3) : '—' }}</td>
              <td class="py-2.5 px-3 text-right font-bold" :class="t.status === 'tp1' ? 'text-green-400' : 'text-red-400'">{{ t.status?.toUpperCase() }}</td>
              <td class="py-2.5 px-3 text-right text-gray-500">{{ t.closedAt ? timeAgo(t.closedAt) : '—' }}<i class="pi pi-chart-line text-rose-500 text-xs ml-2" /></td>
            </tr>
          </tbody>
        </table>
      </div>
        ...
        <div v-if="selected" class="fixed inset-0 z-50 flex items-start justify-center p-4 pt-8 overflow-y-auto" @click.self="closeDialog">
          ...
          <div class="relative z-10 w-full max-w-5xl rounded-2xl bg-[#0d0f14] border border-white/10 shadow-2xl mb-8">
            <div class="flex items-center gap-3 px-6 py-4 border-b border-white/8 sticky top-0 z-10 bg-[#0d0f14] rounded-t-2xl">
              ...
              <span class="text-xs font-mono px-2 py-1 rounded bg-rose-500/15 text-rose-300">{{ selected.patternName?.replace('_LONG','').replace('_SHORT','').replace(/_/g,' ') }}</span>
            ...
            <div class="p-6 space-y-5">
              ...
              <div class="flex flex-wrap gap-3 text-xs font-mono">
                ...
                <span class="flex items-center gap-2"><span class="w-4 h-3 rounded-sm inline-block" style="background: rgba(34,197,94,0.25)" />TP {{ selected.takeProfit1 ?? selected.takeProfit }} (+{{ selected.tpPct?.toFixed(2) }}%)</span>
              </div>
              <div class="grid grid-cols-3 gap-4">
                <div class="rounded-xl bg-black/30 border border-white/5 p-4 space-y-3">
                  ...
                  <div><div class="text-xs text-green-400/70 mb-0.5">Take Profit</div><div class="text-xl font-black text-green-400 font-mono">{{ selected.takeProfit1 ?? selected.takeProfit }}<span class="text-sm text-green-500/60 ml-1">+{{ selected.tpPct?.toFixed(2) }}%</span></div></div>
                ...
                <div class="rounded-xl bg-black/30 border border-white/5 p-4 space-y-3">
                  ...
                  <div><div class="text-xs text-gray-500 mb-0.5">Pattern Name</div><div class="text-lg font-black text-rose-400 font-mono">{{ selected.patternName }}</div></div>
                  <div><div class="text-xs text-gray-500 mb-0.5">Break Strength %</div><div class="text-xl font-black text-white font-mono">{{ selected.breakStrengthPct?.toFixed(3) }}%</div></div>
                  <div><div class="text-xs text-gray-500 mb-0.5">Range ATR</div><div class="text-xl font-black text-white font-mono">{{ selected.rangeAtr?.toFixed(4) }}</div></div>
                  <div><div class="text-xs text-gray-500 mb-0.5">Volume Ratio</div><div class="text-lg font-black text-white font-mono">{{ selected.volumeRatio?.toFixed(3) }}x</div></div>
                </div>
                <div class="rounded-xl bg-black/30 border border-white/5 p-4 space-y-3">
                  ...
                  <div class="text-center py-2 border-t border-white/5">
                    <div class="text-xs text-gray-500 mb-1">Break Strength</div>
                    <div class="text-3xl font-black font-mono text-rose-400">{{ selected.breakStrengthPct?.toFixed(3) }}%</div>
                  </div>
                </div>
              </div>
...
import { useImpulse100Store } from '@/stores/impulse-100'

const store         = useImpulse100Store()
const analytics     = computed(() => store.analytics)
const impulseStatus = computed(() => store.status)
const openTrades    = computed(() => store.openTrades)
const closedTrades  = computed(() => store.closedTrades)
const cfg           = computed(() => store.analytics?.config ?? {} as any)
...
function normalizeDialogItem(item: any) {
  ...
  const takeProfit1 = item.takeProfit1 ?? item.takeProfit
  const reasons = item.reasons ?? [`Trade ${item.direction}`, `Entry ${item.entry}`, `SL ${stopLoss}`, `TP ${takeProfit1}`]
  return {
    ...item, stopLoss, takeProfit1,
    ...
    breakStrengthPct: Number(item.breakStrengthPct ?? 0),
    rangeAtr: Number(item.rangeAtr ?? 0),
...
function chartCandlesUrl(sig: any) {
  ...
  return from ? `/api/impulse-100/candles/${sym}?limit=120&from=${from}${live}` : `/api/impulse-100/candles/${sym}?limit=120${live}`
}
...
function applyChartData(candles: any[]) {
  ...
  chartState.tpZone.setData([{ time: firstTime, value: chartState.sig.takeProfit1 }, { time: lastTime as any, value: chartState.sig.takeProfit1 }])
...
async function buildChart(sig: any) {
  ...
  tpZone.setData([{ time: firstTime, value: sig.takeProfit1 }, { time: lastTime as any, value: sig.takeProfit1 }])
  ...
  addLine(sig.takeProfit1, '#22c55e', LineStyle.Dashed, 'TP')
...
onUnmounted(() => { stopChartLive(); if (chartInstance) chartInstance.remove() })
</script>
