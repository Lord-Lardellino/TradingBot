<template>
  <div class="p-3 sm:p-6 space-y-4">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
      <div>
        <h1 class="text-xl font-bold text-white flex items-center gap-2">
          <span class="inline-block w-2.5 h-2.5 rounded-full bg-orange-400 animate-pulse" />
          Engulfing Scanner 5m
          <span class="text-xs font-normal text-gray-500">Engulfing body + volume spike · 5m · Top 100 coppie</span>
        </h1>
        <p class="text-xs text-gray-500 mt-0.5">
          Scan ogni 5 minuti ·
          <span :class="status?.isScanning ? 'text-orange-400 animate-pulse' : 'text-gray-600'">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
      ...
      <div class="stat-card text-center">
        <div class="text-3xl font-bold font-mono text-orange-400">{{ analytics?.openTrades ?? 0 }}</div>
    ...
    <div class="stat-card">
      <div class="flex flex-wrap gap-2 items-center">
        ...
        <button @click="toggleEnabled"
          :class="['text-sm px-3 py-1 rounded-full font-medium transition', analytics?.config?.enabled ? 'bg-orange-500/20 text-orange-400' : 'bg-surface-200 text-gray-500']">
        ...
        <span class="text-xs text-gray-600 font-mono ml-2">
          ...
          · engulf ≥{{ analytics?.config?.minEngulfRatio ?? 1.5 }}x
          · vol ≥{{ analytics?.config?.minVolumeRatio ?? 1.5 }}x
      ...
      <div v-if="showConfig" class="mt-3 pt-3 border-t border-white/5 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
        ...
        <div class="flex flex-col gap-1">
          <label class="text-xs text-gray-500">Min Engulf Ratio</label>
          <InputNumber v-model="cfg.minEngulfRatio" :min="0" :step="0.01" :maxFractionDigits="2" @update:modelValue="v => store.updateConfig({ minEngulfRatio: v })" />
        </div>
        <div class="flex flex-col gap-1">
          <label class="text-xs text-gray-500">Min Body Ratio</label>
          <InputNumber v-model="cfg.minBodyRatio" :min="0" :step="0.01" :maxFractionDigits="2" @update:modelValue="v => store.updateConfig({ minBodyRatio: v })" />
        </div>
        <div class="flex flex-col gap-1">
      ...
      <div v-if="status?.top50Sample?.length" class="mt-3 flex flex-wrap gap-2">
        <span v-for="sym in status.top50Sample" :key="sym"
          class="text-xs font-mono px-2.5 py-1 rounded-full bg-orange-500/10 text-orange-400">
          {{ sym }}
        </span>
        <span class="text-xs text-gray-600 font-mono py-1">+ altri {{ (status?.symbols ?? 100) - (status?.top50Sample?.length ?? 0) }}</span>
      </div>
    </div>
    ...
    <div class="stat-card space-y-2">
      ...
      <div v-if="!store.signals.length" class="text-sm text-gray-600 py-6 text-center">
        Nessun segnale — scan ogni 5 minuti
      </div>
      <div v-else class="space-y-1.5">
        <button v-for="sig in store.signals.slice(0, 30)" :key="sig.id"
          ...
          <span class="text-xs font-mono px-2 py-0.5 rounded-full bg-orange-500/15 text-orange-300">
            engulf {{ sig.engulfRatio?.toFixed(2) }}x
          </span>
          ...
          <i class="pi pi-chart-line text-orange-500 text-sm" />
        </button>
      </div>
    </div>
    ...
    <div v-if="openTrades.length" class="stat-card space-y-2">
      ...
      <button v-for="t in openTrades" :key="t.id" type="button"
        class="w-full text-left flex items-center gap-3 px-4 py-3 rounded-xl bg-surface-200 border border-white/5 hover:bg-black/35 hover:border-orange-400/25 transition flex-wrap"
        ...
        <span class="text-xs font-mono text-orange-300 px-2 py-0.5 rounded-full bg-orange-500/10">
          engulf {{ Number(t.engulfRatio).toFixed(2) }}x
        </span>
        <div class="flex gap-4 text-sm font-mono">
          ...
          <span v-if="store.positions[t.id]?.currentR != null" class="text-orange-400 font-bold">
        ...
        <table class="w-full text-sm font-mono">
          <thead>
            <tr class="text-xs text-gray-500 uppercase tracking-wider border-b border-white/5">
              ...
              <th class="text-right py-2 px-3">Engulf</th>
          ...
          <tbody>
            <tr v-for="t in closedTrades.slice(0, 100)" :key="t.id"
              ...
              <td class="py-2.5 px-3 text-right text-orange-400">{{ Number(t.engulfRatio).toFixed(2) }}x</td>
              ...
              <td class="py-2.5 px-3 text-right text-gray-500">
                ...
                <i class="pi pi-chart-line text-orange-500 text-xs ml-2" />
              </td>
            </tr>
          </tbody>
        </table>
        ...
        <div v-if="selected" class="fixed inset-0 z-50 flex items-start justify-center p-4 pt-8 overflow-y-auto" @click.self="closeDialog">
          ...
          <div class="relative z-10 w-full max-w-5xl rounded-2xl bg-[#0d0f14] border border-white/10 shadow-2xl mb-8">
            <div class="flex items-center gap-3 px-6 py-4 border-b border-white/8 sticky top-0 z-10 bg-[#0d0f14] rounded-t-2xl">
              ...
              <span class="text-xs font-mono px-2 py-1 rounded bg-orange-500/15 text-orange-300">
                engulf {{ selected.engulfRatio?.toFixed(2) }}x
              </span>
            ...
            <div class="p-6 space-y-5">
              ...
              <div class="grid grid-cols-3 gap-4">
                ...
                <!-- Engulfing data -->
                <div class="rounded-xl bg-black/30 border border-white/5 p-4 space-y-3">
                  <div class="text-xs text-gray-500 uppercase tracking-wider font-semibold">Engulfing</div>
                  <div><div class="text-xs text-gray-500 mb-0.5">Engulf Ratio</div><div class="text-2xl font-black text-orange-400 font-mono">{{ selected.engulfRatio?.toFixed(3) }}x</div></div>
                  ...
                  <div><div class="text-xs text-gray-500 mb-0.5">Delta Ratio</div><div :class="['text-xl font-black font-mono', selected.deltaRatio >= 0 ? 'text-green-400' : 'text-red-400']">{{ selected.deltaRatio >= 0 ? '+' : '' }}{{ (selected.deltaRatio * 100)?.toFixed(0) }}%</div></div>
                </div>
                <!-- Order Flow -->
                <div class="rounded-xl bg-black/30 border border-white/5 p-4 space-y-3">
                  ...
                  <div class="text-center py-2 border-t border-white/5">
                    <div class="text-xs text-gray-500 mb-1">Engulf ratio</div>
                    <div class="text-3xl font-black font-mono text-orange-400">{{ selected.engulfRatio?.toFixed(2) }}x</div>
                  </div>
                </div>
              </div>
...
import { useEngulfingStore } from '@/stores/engulfing'

const store        = useEngulfingStore()
...
async function openDialog(sig: any) {
  selected.value = normalizeDialogItem(sig)
  await nextTick()
...
function normalizeDialogItem(item: any) {
  ...
  const reasons = item.reasons ?? [`Trade ${item.direction}`, `Entry ${item.entry}`, `SL ${stopLoss}`, `TP ${item.takeProfit}`, `Score ${item.score ?? '-'}`]
  return {
    ...item, stopLoss,
    ...
    engulfRatio: Number(item.engulfRatio ?? 0),
...
function closeDialog() {
  ...
  chartState = null; selected.value = null
}
...
function chartCandlesUrl(sig: any) {
  ...
  return from ? `/api/engulfing/candles/${sym}?limit=120&from=${from}${live}` : `/api/engulfing/candles/${sym}?limit=120${live}`
}
...
function applyChartData(candles: any[]) {
  ...
  const lastTime = (candles[candles.length - 1]?.time ?? 0) + 300
...
function stopChartLive() {
  ...
  chartRefreshTimer = null; chartRefreshBusy = false
...
function startChartLive(sig: any) {
  ...
  chartRefreshTimer = setInterval(async () => {
    ...
    try { const candles = await fetchChartCandles(sig); applyChartData(candles); chartInstance?.timeScale().scrollToRealTime() }
    catch {} finally { chartRefreshBusy = false }
  }, 3000)
}
...
async function buildChart(sig: any) {
  ...
  const chart = createChart(chartContainer.value, {
    ...
    width: chartContainer.value.clientWidth, height: chartContainer.value.clientHeight,
  })
  chartInstance = chart
  ...
  const addLine = (price: number, color: string, style: LineStyle, title: string) => {
    if (!price) return
    const s = chart.addLineSeries({ color, lineWidth: 1 as any, lineStyle: style, priceLineVisible: false, lastValueVisible: false, title })
