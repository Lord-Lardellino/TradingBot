<template>
  <div class="p-3 sm:p-6 space-y-4">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
      <div>
        <h1 class="text-xl font-bold text-white flex items-center gap-2">
          ...
          MTF Scalper 5m
          <span class="text-xs font-normal text-gray-500">EMA9/21/200(5m) + HTF bias EMA200(30m) · 5m · Top 100 coppie</span>
        </h1>
        <p class="text-xs text-gray-500 mt-0.5">
          Scan ogni 5 minuti ·
          <span :class="status?.isScanning ? 'text-cyan-400 animate-pulse' : 'text-gray-600'">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
      ...
      <div class="stat-card text-center">
        <div class="text-3xl font-bold font-mono text-cyan-400">{{ analytics?.openTrades ?? 0 }}</div>
    ...
    <div class="stat-card">
      <div class="flex flex-wrap gap-2 items-center">
        ...
        <button @click="toggleEnabled"
          :class="['text-sm px-3 py-1 rounded-full font-medium transition', analytics?.config?.enabled ? 'bg-cyan-500/20 text-cyan-400' : 'bg-surface-200 text-gray-500']">
        ...
        <span class="text-xs text-gray-600 font-mono ml-2">
          ...
          · body ≥{{ analytics?.config?.minBodyRatio ?? 0.5 }}
      ...
      <div v-if="showConfig" class="mt-3 pt-3 border-t border-white/5 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
        ...
        <div class="flex flex-col gap-1">
          <label class="text-xs text-gray-500">Max Close Wick</label>
          <InputNumber v-model="cfg.maxCloseWick" :min="0" :step="0.01" :maxFractionDigits="2" @update:modelValue="v => store.updateConfig({ maxCloseWick: v })" />
        </div>
        <div class="flex flex-col gap-1">
      ...
      <div v-if="status?.top50Sample?.length" class="mt-3 flex flex-wrap gap-2">
        <span v-for="sym in status.top50Sample" :key="sym" class="text-xs font-mono px-2.5 py-1 rounded-full bg-cyan-500/10 text-cyan-400">{{ sym }}</span>
    ...
    <div class="stat-card space-y-2">
      ...
      <div v-if="!store.signals.length" class="text-sm text-gray-600 py-6 text-center">Nessun segnale — scan ogni 5 minuti</div>
      <div v-else class="space-y-1.5">
        <button v-for="sig in store.signals.slice(0, 30)" :key="sig.id"
          ...
          <span class="text-xs font-mono px-2 py-0.5 rounded-full bg-cyan-500/15 text-cyan-300">body {{ sig.bodyRatio?.toFixed(2) }}</span>
          ...
          <i class="pi pi-chart-line text-cyan-500 text-sm" />
        </button>
      </div>
    </div>
    ...
    <div v-if="openTrades.length" class="stat-card space-y-2">
      ...
      <button v-for="t in openTrades" :key="t.id" type="button"
        class="w-full text-left flex items-center gap-3 px-4 py-3 rounded-xl bg-surface-200 border border-white/5 hover:bg-black/35 hover:border-cyan-400/25 transition flex-wrap"
        ...
        <span class="text-xs font-mono text-cyan-300 px-2 py-0.5 rounded-full bg-cyan-500/10">body {{ Number(t.bodyRatio).toFixed(2) }}</span>
        <div class="flex gap-4 text-sm font-mono">
          ...
          <span v-if="store.positions[t.id]?.currentR != null" class="text-cyan-400 font-bold">{{ store.positions[t.id].currentR }}R</span>
        </div>
        ...
        <table class="w-full text-sm font-mono">
          <thead>
            <tr class="text-xs text-gray-500 uppercase tracking-wider border-b border-white/5">
              ...
              <th class="text-right py-2 px-3">Body</th>
          ...
          <tbody>
            <tr v-for="t in closedTrades.slice(0, 100)" :key="t.id"
              ...
              <td class="py-2.5 px-3 text-right text-cyan-400">{{ Number(t.bodyRatio).toFixed(2) }}</td>
              ...
              <td class="py-2.5 px-3 text-right text-gray-500">{{ t.closedAt ? timeAgo(t.closedAt) : '—' }}<i class="pi pi-chart-line text-cyan-500 text-xs ml-2" /></td>
            </tr>
          </tbody>
        </table>
      </div>
        ...
        <div v-if="selected" class="fixed inset-0 z-50 flex items-start justify-center p-4 pt-8 overflow-y-auto" @click.self="closeDialog">
          ...
          <div class="relative z-10 w-full max-w-5xl rounded-2xl bg-[#0d0f14] border border-white/10 shadow-2xl mb-8">
            <div class="flex items-center gap-3 px-6 py-4 border-b border-white/8 sticky top-0 z-10 bg-[#0d0f14] rounded-t-2xl">
              ...
              <span class="text-xs font-mono px-2 py-1 rounded bg-cyan-500/15 text-cyan-300">body {{ selected.bodyRatio?.toFixed(2) }}</span>
            ...
            <div class="p-6 space-y-5">
              ...
              <div class="grid grid-cols-3 gap-4">
                ...
                <div class="rounded-xl bg-black/30 border border-white/5 p-4 space-y-3">
                  <div class="text-xs text-gray-500 uppercase tracking-wider font-semibold">MTF</div>
                  <div><div class="text-xs text-gray-500 mb-0.5">Body Ratio</div><div class="text-2xl font-black text-cyan-400 font-mono">{{ selected.bodyRatio?.toFixed(3) }}</div></div>
                ...
                <div class="rounded-xl bg-black/30 border border-white/5 p-4 space-y-3">
                  ...
                  <div class="text-center py-2 border-t border-white/5">
                    <div class="text-xs text-gray-500 mb-1">Body ratio</div>
                    <div class="text-3xl font-black font-mono text-cyan-400">{{ selected.bodyRatio?.toFixed(3) }}</div>
                  </div>
                </div>
              </div>
...
import { useMtfScalperStore } from '@/stores/mtf-scalper'

const store        = useMtfScalperStore()
...
function normalizeDialogItem(item: any) {
  ...
  return {
    ...
    bodyRatio: Number(item.bodyRatio ?? 0),
...
function chartCandlesUrl(sig: any) {
  ...
  return from ? `/api/mtf-scalper/candles/${sym}?limit=120&from=${from}${live}` : `/api/mtf-scalper/candles/${sym}?limit=120${live}`
}
...
function applyChartData(candles: any[]) {
  ...
  const firstTime = candles[0]?.time as any; const lastTime = (candles[candles.length - 1]?.time ?? 0) + 300
...
async function buildChart(sig: any) {
  ...
  const firstTime = candles[0]?.time as any; const lastTime = (candles[candles.length - 1]?.time ?? 0) + 300; const isLong = sig.direction === 'LONG'
