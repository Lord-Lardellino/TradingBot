<template>
  <div class="p-3 sm:p-6 space-y-4">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
      <div>
        <h1 class="text-xl font-bold text-white flex items-center gap-2">
          <span class="inline-block w-2.5 h-2.5 rounded-full bg-purple-400 animate-pulse" />
          Fibonacci Scanner 1m
          <span class="text-xs font-normal text-gray-500">Fib retracement + VWAP + delta · 1m · Coppie fisse</span>
        </h1>
        <p class="text-xs text-gray-500 mt-0.5">
          Scan ogni minuto ·
          <span :class="status?.isScanning ? 'text-purple-400 animate-pulse' : 'text-gray-600'">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
      ...
      <div class="stat-card text-center">
        <div class="text-3xl font-bold font-mono text-purple-400">{{ analytics?.openTrades ?? 0 }}</div>
    ...
    <div class="stat-card">
      <div class="flex flex-wrap gap-2 items-center">
        ...
        <button @click="toggleEnabled"
          :class="['text-sm px-3 py-1 rounded-full font-medium transition', analytics?.config?.enabled ? 'bg-purple-500/20 text-purple-400' : 'bg-surface-200 text-gray-500']">
        ...
        <span class="text-xs text-gray-600 font-mono ml-2">
          ...
          · swing ≥{{ analytics?.config?.minSwingPct ?? 0.5 }}%
          · prox {{ analytics?.config?.proxPct ?? 0.2 }}%
      ...
      <div v-if="showConfig" class="mt-3 pt-3 border-t border-white/5 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
        ...
        <div class="flex flex-col gap-1">
          <label class="text-xs text-gray-500">Min Swing %</label>
          <InputNumber v-model="cfg.minSwingPct" :min="0" :step="0.01" :maxFractionDigits="2" @update:modelValue="v => store.updateConfig({ minSwingPct: v })" />
        </div>
        <div class="flex flex-col gap-1">
          <label class="text-xs text-gray-500">Prox %</label>
          <InputNumber v-model="cfg.proxPct" :min="0" :step="0.01" :maxFractionDigits="2" @update:modelValue="v => store.updateConfig({ proxPct: v })" />
        </div>
        <div class="flex flex-col gap-1">
        ...
        <div class="flex flex-col gap-1">
          <label class="text-xs text-gray-500">Swing Pivot N</label>
          <InputNumber v-model="cfg.swingPivotN" :min="1" :step="1" :maxFractionDigits="0" @update:modelValue="v => store.updateConfig({ swingPivotN: v })" />
        </div>
        <div class="flex flex-col gap-1">
      ...
      <div v-if="status?.top50Sample?.length" class="mt-3 flex flex-wrap gap-2">
        <span v-for="sym in status.top50Sample" :key="sym" class="text-xs font-mono px-2.5 py-1 rounded-full bg-purple-500/10 text-purple-400">{{ sym }}</span>
      </div>
    </div>
    ...
    <div class="stat-card space-y-2">
      ...
      <div v-else class="space-y-1.5">
        <button v-for="sig in store.signals.slice(0, 30)" :key="sig.id"
          ...
          <span class="text-xs font-mono px-2 py-0.5 rounded-full bg-purple-500/15 text-purple-300">{{ sig.fibLevelName }}</span>
          ...
          <i class="pi pi-chart-line text-purple-500 text-sm" />
        </button>
      </div>
    </div>
    ...
    <div v-if="openTrades.length" class="stat-card space-y-2">
      ...
      <button v-for="t in openTrades" :key="t.id" type="button"
        class="w-full text-left flex items-center gap-3 px-4 py-3 rounded-xl bg-surface-200 border border-white/5 hover:bg-black/35 hover:border-purple-400/25 transition flex-wrap"
        ...
        <span class="text-xs font-mono text-purple-300 px-2 py-0.5 rounded-full bg-purple-500/10">{{ t.fibLevelName }}</span>
        <div class="flex gap-4 text-sm font-mono">
          ...
          <span v-if="store.positions[t.id]?.currentR != null" class="text-purple-400 font-bold">{{ store.positions[t.id].currentR }}R</span>
        </div>
        ...
        <table class="w-full text-sm font-mono">
          <thead>
            <tr class="text-xs text-gray-500 uppercase tracking-wider border-b border-white/5">
              ...
              <th class="text-right py-2 px-3">Fib Level</th>
          ...
          <tbody>
            <tr v-for="t in closedTrades.slice(0, 100)" :key="t.id"
              ...
              <td class="py-2.5 px-3 text-right text-purple-400">{{ t.fibLevelName }}</td>
              ...
              <td class="py-2.5 px-3 text-right text-gray-500">{{ t.closedAt ? timeAgo(t.closedAt) : '—' }}<i class="pi pi-chart-line text-purple-500 text-xs ml-2" /></td>
            </tr>
          </tbody>
        </table>
      </div>
        ...
        <div v-if="selected" class="fixed inset-0 z-50 flex items-start justify-center p-4 pt-8 overflow-y-auto" @click.self="closeDialog">
          ...
          <div class="relative z-10 w-full max-w-5xl rounded-2xl bg-[#0d0f14] border border-white/10 shadow-2xl mb-8">
            <div class="flex items-center gap-3 px-6 py-4 border-b border-white/8 sticky top-0 z-10 bg-[#0d0f14] rounded-t-2xl">
              ...
              <span class="text-xs font-mono px-2 py-1 rounded bg-purple-500/15 text-purple-300">{{ selected.fibLevelName }}</span>
            ...
            <div class="p-6 space-y-5">
              ...
              <div class="grid grid-cols-3 gap-4">
                ...
                <div class="rounded-xl bg-black/30 border border-white/5 p-4 space-y-3">
                  <div class="text-xs text-gray-500 uppercase tracking-wider font-semibold">Fibonacci</div>
                  <div><div class="text-xs text-gray-500 mb-0.5">Fib Level</div><div class="text-2xl font-black text-purple-400 font-mono">{{ selected.fibLevel?.toFixed(3) }}</div></div>
                  <div><div class="text-xs text-gray-500 mb-0.5">Level Name</div><div class="text-xl font-black text-white font-mono">{{ selected.fibLevelName }}</div></div>
                  <div><div class="text-xs text-gray-500 mb-0.5">Fib Price</div><div class="text-xl font-black text-white font-mono">{{ selected.fibPrice }}</div></div>
                  <div><div class="text-xs text-gray-500 mb-0.5">Swing Range</div><div class="text-lg font-black text-purple-300 font-mono">{{ selected.swingRange?.toFixed(3) }}%</div></div>
                </div>
                <div class="rounded-xl bg-black/30 border border-white/5 p-4 space-y-3">
                  ...
                  <div class="text-center py-2 border-t border-white/5">
                    <div class="text-xs text-gray-500 mb-1">Fib level</div>
                    <div class="text-3xl font-black font-mono text-purple-400">{{ selected.fibLevel?.toFixed(3) }}</div>
                  </div>
                </div>
              </div>
...
import { useFibScannerStore } from '@/stores/fib-scanner'

const store        = useFibScannerStore()
...
function normalizeDialogItem(item: any) {
  ...
  return {
    ...
    fibLevel: Number(item.fibLevel ?? 0),
    fibLevelName: item.fibLevelName ?? '',
    fibPrice: item.fibPrice ?? 0,
    swingRange: Number(item.swingRange ?? 0),
...
function chartCandlesUrl(sig: any) {
  ...
  return from ? `/api/fib-scanner/candles/${sym}?limit=120&from=${from}${live}` : `/api/fib-scanner/candles/${sym}?limit=120${live}`
}
...
async function buildChart(sig: any) {
  ...
  if (sig.fibPrice) addLine(Number(sig.fibPrice), '#a855f7', LineStyle.Dotted, 'Fib')
