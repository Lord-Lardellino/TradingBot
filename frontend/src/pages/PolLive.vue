<template>
  <div class="p-3 sm:p-6 space-y-4">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
      <div>
        <h1 class="text-xl font-bold text-white flex items-center gap-2">
          <span class="inline-block w-2.5 h-2.5 rounded-full bg-teal-400 animate-pulse" />
          POL Live 1m
          <span class="text-xs font-normal text-gray-500">Base breakout + impulse close · POL/USDT · 1m</span>
        </h1>
        <p class="text-xs text-gray-500 mt-0.5">
          Scan ogni minuto ·
          <span :class="polStatus?.isScanning ? 'text-teal-400 animate-pulse' : 'text-gray-600'">
            {{ polStatus?.isScanning ? '⚡ Scanning...' : '● Idle' }}
          </span>
          <span v-if="polStatus?.lastScanAt" class="ml-2">· {{ timeAgo(polStatus.lastScanAt) }}</span>
        </p>
      </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
      ...
      <div class="stat-card text-center">
        <div class="text-3xl font-bold font-mono text-teal-400">{{ analytics?.openTrades ?? 0 }}</div>
    ...
    <div class="stat-card">
      <div class="flex flex-wrap gap-2 items-center">
        ...
        <button @click="toggleEnabled"
          :class="['text-sm px-3 py-1 rounded-full font-medium transition', store.config?.enabled ? 'bg-teal-500/20 text-teal-400' : 'bg-surface-200 text-gray-500']">
          {{ store.config?.enabled ? '⚡ Scan ON' : '⏸ Scan OFF' }}
        </button>
        <span class="text-xs text-gray-600 font-mono ml-2">
          RR {{ store.config?.tpRr ?? 2 }} · risk {{ store.config?.riskUsdt ?? 1 }}$
          · breakout ≥{{ store.config?.breakoutLookback ?? 20 }}c
        </span>
        <button @click="showConfig = !showConfig" class="ml-auto text-gray-500 hover:text-white text-sm">⚙️</button>
      </div>
      ...
      <div v-if="showConfig" class="mt-3 pt-3 border-t border-white/5 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
        ...
        <div class="flex flex-col gap-1">
          <label class="text-xs text-gray-500">Breakout Lookback</label>
          <InputNumber v-model="cfg.breakoutLookback" :min="1" :step="1" :maxFractionDigits="0" @update:modelValue="v => store.updateConfig({ breakoutLookback: v })" />
        </div>
    ...
    <!-- Debug -->
    <div v-if="polStatus?.debug && Object.keys(polStatus.debug).length" class="stat-card">
      <div class="text-xs text-gray-500 mb-2">Debug filtri</div>
      <div class="flex flex-wrap gap-2">
        <span v-for="(v, k) in polStatus.debug" :key="k"
    ...
    <div class="stat-card space-y-2">
      ...
      <div v-else class="space-y-1.5">
        <button v-for="sig in store.signals.slice(0, 30)" :key="sig.id"
          ...
          <span class="text-xs font-mono px-2 py-0.5 rounded-full bg-teal-500/15 text-teal-300">{{ sig.signalType }}</span>
          ...
          <i class="pi pi-chart-line text-teal-500 text-sm" />
        </button>
      </div>
    </div>
    ...
    <div v-if="store.openTrades.length" class="stat-card space-y-2">
      ...
      <button v-for="t in store.openTrades" :key="t.id" type="button"
        class="w-full text-left flex items-center gap-3 px-4 py-3 rounded-xl bg-surface-200 border border-white/5 hover:bg-black/35 hover:border-teal-400/25 transition flex-wrap"
        ...
        <i class="ml-auto pi pi-chart-line text-teal-500 text-sm" />
      </button>
    </div>
    ...
    <div class="stat-card space-y-2">
      ...
      <div v-if="!closedTradesList.length" class="text-sm text-gray-600 py-6 text-center">Nessun trade chiuso ancora</div>
      <div v-else class="overflow-x-auto">
        <table class="w-full text-sm font-mono">
          ...
          <tbody>
            <tr v-for="t in closedTradesList.slice(0, 100)" :key="t.id"
              ...
              <td class="py-2.5 px-3 text-right text-yellow-400">—</td>
              ...
              <td class="py-2.5 px-3 text-right text-gray-500">{{ t.closedAt ? timeAgo(t.closedAt) : '—' }}<i class="pi pi-chart-line text-teal-500 text-xs ml-2" /></td>
            </tr>
          </tbody>
        </table>
      </div>
        ...
        <div v-if="selected" class="fixed inset-0 z-50 flex items-start justify-center p-4 pt-8 overflow-y-auto" @click.self="closeDialog">
          ...
          <div class="relative z-10 w-full max-w-5xl rounded-2xl bg-[#0d0f14] border border-white/10 shadow-2xl mb-8">
            <div class="flex items-center gap-3 px-6 py-4 border-b border-white/8 sticky top-0 z-10 bg-[#0d0f14] rounded-t-2xl">
              ...
              <span class="text-xs font-mono px-2 py-1 rounded bg-teal-500/15 text-teal-300">{{ selected.signalType }}</span>
              ...
              <div class="grid grid-cols-3 gap-4">
                <div class="rounded-xl bg-black/30 border border-white/5 p-4 space-y-3">
                  ...
                  <div class="pt-2 border-t border-white/5 flex gap-4 text-center">
                    ...
                    <div><div class="text-xs text-gray-500">Leva</div><div class="text-lg font-black text-white">{{ selected.suggestedLeverage ?? selected.leverage }}x</div></div>
                ...
                <div class="rounded-xl bg-black/30 border border-white/5 p-4 space-y-3">
                  ...
                  <div><div class="text-xs text-gray-500 mb-0.5">Signal Type</div><div class="text-2xl font-black text-teal-400 font-mono">{{ selected.signalType }}</div></div>
                  <div><div class="text-xs text-gray-500 mb-0.5">Base Range %</div><div class="text-xl font-black text-white font-mono">{{ selected.baseRangePct?.toFixed(3) }}%</div></div>
                  <div><div class="text-xs text-gray-500 mb-0.5">Range ATR</div><div class="text-lg font-black text-teal-300 font-mono">{{ selected.rangeAtr?.toFixed(4) }}</div></div>
                  <div><div class="text-xs text-gray-500 mb-0.5">Spread %</div><div class="text-lg font-black text-white font-mono">{{ selected.spreadPct?.toFixed(4) }}</div></div>
                </div>
                <div class="rounded-xl bg-black/30 border border-white/5 p-4 space-y-3">
                  <div class="text-xs text-gray-500 uppercase tracking-wider font-semibold">Order Flow</div>
                  <div class="text-center py-2">
                    <div class="text-xs text-gray-500 mb-1">Signal Type</div>
                    <div class="text-2xl font-black font-mono text-teal-400">{{ selected.signalType }}</div>
                  </div>
                  <div class="text-center py-2 border-t border-white/5">
                    <div class="text-xs text-gray-500 mb-1">Range ATR</div>
                    <div class="text-2xl font-black font-mono text-white">{{ selected.rangeAtr?.toFixed(4) }}</div>
                  </div>
                </div>
              </div>
...
import { usePolLiveStore } from '@/stores/pol-live'

const store        = usePolLiveStore()
const analytics    = computed(() => store.analytics)
const polStatus    = computed(() => store.status)
// config is inside status
const cfg          = computed(() => store.config ?? {} as any)
const showConfig   = ref(false)
// closed trades = all trades that are not open
const closedTradesList = computed(() => store.trades.filter(t => !['open', 'pending'].includes(t.status)))
...
function normalizeDialogItem(item: any) {
  ...
  const takeProfit = Number(item.takeProfit ?? 0)
  ...
  return {
    ...
    baseRangePct: Number(item.baseRangePct ?? 0),
    rangeAtr: Number(item.rangeAtr ?? 0),
    spreadPct: Number(item.spreadPct ?? 0),
    reasons,
  }
}
...
function chartCandlesUrl(sig: any) {
  ...
  return from ? `/api/pol-live/candles/${sym}?limit=120&from=${from}${live}` : `/api/pol-live/candles/${sym}?limit=120${live}`
}
...
async function toggleEnabled() { await store.updateConfig({ enabled: !store.config?.enabled }) }
