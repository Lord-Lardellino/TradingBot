  ...
  <div class="p-3 sm:p-6 space-y-4">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
      <div>
        <h1 class="text-xl font-bold text-white flex items-center gap-2">
          <span class="inline-block w-2.5 h-2.5 rounded-full bg-indigo-400 animate-pulse" />
          Inst5m Scanner
          <span class="text-xs font-normal text-gray-500">Liquidity Sweep · FVG · BOS — 5m istituzionale</span>
        </h1>
        <p class="text-xs text-gray-500 mt-0.5">
          {{ analytics?.scannerStatus.scannedPairs ?? 0 }} coppie ·
          {{ analytics?.scannerStatus.lastRawSignals ?? 0 }} segnali ciclo ·
          scan ogni 1 min · batch paralleli
        </p>
      </div>
      <div class="flex items-center gap-2">
        <!-- Pills -->
        <span class="text-[10px] font-mono px-2 py-1 rounded-full bg-surface-200 text-gray-400">
          minScore: {{ analytics?.config?.minScore ?? 50 }}
        </span>
        <span class="text-[10px] font-mono px-2 py-1 rounded-full bg-surface-200 text-gray-400">
          RR: 1:{{ analytics?.config?.tpRr ?? 3 }}
        </span>
        <span :class="['text-[10px] font-mono px-2 py-1 rounded-full', status.isScanning ? 'bg-indigo-500/20 text-indigo-400 animate-pulse' : 'bg-surface-200 text-gray-500']">
    ...
    <div class="grid grid-cols-2 md:grid-cols-5 gap-3">
      ...
      <div class="stat-card text-center">
        ...
        <div class="text-xl font-bold font-mono text-indigo-400">€{{ analytics?.capital?.toFixed(2) ?? '500.00' }}</div>
        <div class="text-xs text-gray-600 mt-0.5">start €500</div>
      </div>
      <div class="stat-card text-center">
        <div class="text-xs text-gray-500 mb-1">Pattern breakdown</div>
        <div class="text-xs font-mono space-y-0.5 mt-1">
          <div v-for="(v, k) in analytics?.byPattern" :key="k" class="flex items-center justify-between gap-2">
            <span class="text-gray-400">{{ k }}</span>
            <span class="text-green-400">{{ v.tp }}W</span>
            <span class="text-red-400">{{ v.sl }}L</span>
          </div>
          <div v-if="!analytics?.byPattern || !Object.keys(analytics.byPattern).length" class="text-gray-600">nessun dato</div>
        </div>
      </div>
    </div>
    ...
    <div class="flex border-b border-white/5 gap-1">
      ...
      >
        {{ tab.label }}
        <span v-if="tab.badge" class="ml-1.5 text-[10px] px-1.5 py-0.5 rounded-full bg-indigo-500/20 text-indigo-400">{{ tab.badge }}</span>
      </button>
    </div>
    ...
    <div v-if="activeTab === 'signals'">
      <div v-if="!recentSignals.length" class="flex flex-col items-center justify-center py-20 text-gray-600">
        ...
        <p class="text-sm">Nessun segnale — scanner in attesa del prossimo ciclo (5 min)</p>
      </div>
      <div v-else class="space-y-2">
        ...
        >
          ...
          <div class="flex-1 min-w-0">
            <a :href="sig.mexcUrl" target="_blank" class="font-semibold text-white text-sm hover:text-indigo-400 transition-colors">
          ...
          <div class="text-right text-xs font-mono hidden md:block">
            ...
            <div class="text-gray-500">Vol <span class="text-indigo-300">×{{ sig.volumeRatio }}</span></div>
          </div>
          <!-- Sparkline -->
    ...
    <div v-if="activeTab === 'config'" class="stat-card space-y-5">
      <h3 class="text-sm font-semibold text-white">Configurazione Scanner</h3>
      ...
      <div class="border-t border-white/5 pt-4 space-y-3">
        <h4 class="text-xs font-semibold text-gray-400 uppercase tracking-wider">Pattern istituzionali attivi</h4>
        <div class="grid md:grid-cols-3 gap-3">
          <div class="bg-surface-200 rounded-xl p-3 space-y-1">
            <div class="text-xs font-semibold text-indigo-400">🎯 Liquidity Sweep</div>
            <div class="text-[11px] text-gray-400">Prezzo spazza swing high/low → chiude dentro. Istituzioni raccolgono liquidità retail e invertono. Wick ≥ 1.5× body + volume spike.</div>
          </div>
          <div class="bg-surface-200 rounded-xl p-3 space-y-1">
            <div class="text-xs font-semibold text-yellow-400">⚡ Fair Value Gap</div>
            <div class="text-[11px] text-gray-400">Gap di 3 candele (squilibrio). Il prezzo torna a riempire la zona e rimbalza. Ritorno al gap ~70% delle volte secondo backtest istituzionali.</div>
          </div>
          <div class="bg-surface-200 rounded-xl p-3 space-y-1">
            <div class="text-xs font-semibold text-green-400">📐 BOS Retest</div>
            <div class="text-[11px] text-gray-400">Break of Structure confermato da volume → prezzo ritesta il livello rotto. Entry sulla reazione. Win rate 65-75% con conferma volume.</div>
          </div>
        </div>
      </div>
    </div>
...
import { useInstScannerStore } from '@/stores/inst-scanner'
...
const store   = useInstScannerStore()
...
const draft = ref({ minScore: 50, atrSlMult: 0.5, tpRr: 3.0, maxConcurrent: 5, autoEnter: true, liveEnabled: false })
watch(analytics, (a) => {
  if (!a?.config) return
