...
@Injectable()
export class InstScanner1hService implements OnModuleInit {
  private readonly logger = new Logger(InstScanner1hService.name);
  private exchange: ccxt.mexc;
  private fastExchange: ccxt.mexc;
  private validSymbols = new Set<string>();
  private recentSignals: Inst1hSignal[] = [];
  ...
  async onModuleInit() {
    ...
    this.logger.log(`[INST1h] ${this.validSymbols.size} coppie · TF=1h`);
  }
  ...
  // 1h candles close at :00 — fire 8s before at :59:52
  @Cron('44 59 * * * *')
  async scan() {
    ...
    try {
      ...
      this.dbg = {};
      this.logger.log(`[INST1h] scansiono ${candidates.length} pair`);

      const openCount = await this.prisma.inst1hSimulatedTrade.count({
      ...
      const cycleSignals: Inst1hSignal[] = [];
      ...
      this.logger.log(`[INST1h DBG] raw=${cycleSignals.length} ${JSON.stringify(this.dbg)}`);
      ...
      const best = [bestLong, bestShort].filter(Boolean) as Inst1hSignal[];
      ...
      for (const sig of toProcess) {
        ...
        this.events.server.emit('inst1h:signal', sig);
        ...
        if (cfg.liveEnabled) {
          ...
          }).catch(err => this.logger.error(`[INST1h LIVE] ${err.message}`));
        }
        emitted++;
        this.logger.log(`[INST1h ✅] ${sig.direction} ${sig.symbol} score=${sig.score} grade=${sig.grade}`);
      }
      this.lastEmitted = emitted;
      this._emitStatus(cfg, candidates.length, cycleSignals.length, emitted);
    } catch (err: any) { this.logger.error(`Inst1h scan: ${err.message}`); }
  ...
  async checkOpenTrades() {
    try {
      const open = await this.prisma.inst1hSimulatedTrade.findMany({
        where: { status: 'open', openedAt: { gte: this.sessionStart } },
      });
      ...
      for (const t of open) {
        try {
          const ohlcv = await this.exchange.fetchOHLCV(t.symbol, '5m', undefined, 3);
          ...
          this.events.server.emit('inst1h:positions', [{
            id: t.id,
            currentPrice: curr,
          ...
          if (!beActive && beHit) {
            await this.prisma.inst1hSimulatedTrade.update({ where: { id: t.id }, data: { stopLoss: t.entry } });
            this.logger.log(`[INST1h BE] ${t.symbol} BE attivato`);
          }
          ...
          await this.prisma.inst1hSimulatedTrade.update({
            where: { id: t.id },
          ...
          if (cfg.autoEnter) {
            await this.prisma.inst1hSimConfig.update({ where: { id: 1 }, data: { startingCapital: parseFloat(capitalAfter.toFixed(4)) } });
          }
          const updated = await this.prisma.inst1hSimulatedTrade.findUnique({ where: { id: t.id } });
          this.events.server.emit('inst1h:trade', updated);
          this.logger.log(`[INST1h CLOSE] ${t.symbol} ${status.toUpperCase()} PnL ${pnl >= 0 ? '+' : ''}€${pnl.toFixed(3)}`);
        } catch { /* skip */ }
      }
    } catch (err: any) { this.logger.error(`Inst1h checkOpen: ${err.message}`); }
  }

  private analyzePair(sym: string, raw: number[][], cfg: any): Inst1hSignal | null {
    const n = raw.length;
    ...
    const leverage = Math.min(10, Math.max(2, Math.round(1 / (slPct / 100) * 0.5)));
  ...
  private async enterSimTrade(sig: Inst1hSignal, cfg: any) {
    const already = await this.prisma.inst1hSimulatedTrade.findFirst({ where: { symbol: sig.symbol, status: 'open' } });
    if (already) return;
    ...
    const trade = await this.prisma.inst1hSimulatedTrade.create({
      data: {
    ...
    this.events.server.emit('inst1h:trade', trade);
  }
  ...
  private _emitStatus(cfg: any, candidates: number, rawSignals: number, emitted: number) {
    this.events.server.emit('inst1h:status', {
  ...
  private async initConfig() {
    const exists = await this.prisma.inst1hSimConfig.findUnique({ where: { id: 1 } });
    if (!exists) await this.prisma.inst1hSimConfig.create({
      data: { id: 1, startingCapital: 500, maxConcurrent: 2, autoEnter: true, minScore: 55, atrSlMult: 0.5, tpRr: 1.5, liveEnabled: false },
    });
  }

  async getConfig() {
    let cfg = await this.prisma.inst1hSimConfig.findUnique({ where: { id: 1 } });
    if (!cfg) cfg = await this.prisma.inst1hSimConfig.create({
      data: { id: 1, startingCapital: 500, maxConcurrent: 2, autoEnter: true, minScore: 55, atrSlMult: 0.5, tpRr: 1.5, liveEnabled: false },
    });
    return cfg;
  }
  ...
  async updateConfig(data: any) {
    return this.prisma.inst1hSimConfig.upsert({
      where: { id: 1 },
      create: { id: 1, startingCapital: 500, maxConcurrent: 2, autoEnter: true, minScore: 55, atrSlMult: 0.5, tpRr: 1.5, liveEnabled: false, ...data },
      update: data,
    });
  }
  ...
  async getAnalytics() {
    const cfg    = await this.getConfig();
    const trades = await this.prisma.inst1hSimulatedTrade.findMany({ orderBy: { openedAt: 'desc' } });
  ...
  async getTrades(limit = 100) {
    return this.prisma.inst1hSimulatedTrade.findMany({ orderBy: { openedAt: 'desc' }, take: limit });
  }

  async resetSim() {
    await this.prisma.inst1hSimulatedTrade.deleteMany({});
    await this.prisma.inst1hSimConfig.update({ where: { id: 1 }, data: { startingCapital: 500 } });
    this.recentSignals = [];
  }
}
