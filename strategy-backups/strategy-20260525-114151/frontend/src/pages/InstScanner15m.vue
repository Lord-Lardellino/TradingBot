  ...
  <div class="p-3 sm:p-6 space-y-4">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
      <div>
        <h1 class="text-xl font-bold text-white flex items-center gap-2">
          ...
          Inst15m Scanner
          <span class="text-xs font-normal text-gray-500">Liquidity Sweep · FVG · BOS — 15m istituzionale</span>
        </h1>
        <p class="text-xs text-gray-500 mt-0.5">
          {{ analytics?.scannerStatus.scannedPairs ?? 0 }} coppie ·
          {{ analytics?.scannerStatus.lastRawSignals ?? 0 }} segnali ciclo ·
          scan ogni 15 min · batch paralleli
        </p>
      </div>
      <div class="flex items-center gap-2">
        ...
        <span :class="['text-[10px] font-mono px-2 py-1 rounded-full', status.isScanning ? 'bg-violet-500/20 text-violet-400 animate-pulse' : 'bg-surface-200 text-gray-500']">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-3">
      ...
      <div class="stat-card text-center">
        ...
        <div class="text-xl font-bold font-mono text-violet-400">€{{ analytics?.capital?.toFixed(2) ?? '500.00' }}</div>
    ...
    <div class="flex border-b border-white/5 gap-1">
      ...
      >
        {{ tab.label }}
        <span v-if="tab.badge" class="ml-1.5 text-[10px] px-1.5 py-0.5 rounded-full bg-violet-500/20 text-violet-400">{{ tab.badge }}</span>
      </button>
    </div>
    ...
    <div v-if="activeTab === 'signals'">
      <div v-if="!recentSignals.length" class="flex flex-col items-center justify-center py-20 text-gray-600">
        ...
        <p class="text-sm">Nessun segnale — scanner in attesa del prossimo ciclo (15 min)</p>
      </div>
      <div v-else class="space-y-2">
        ...
        >
          ...
          <div class="flex-1 min-w-0">
            <a :href="sig.mexcUrl" target="_blank" class="font-semibold text-white text-sm hover:text-violet-400 transition-colors">
          ...
          <div class="text-right text-xs font-mono hidden md:block">
            ...
            <div class="text-gray-500">Vol <span class="text-violet-300">×{{ sig.volumeRatio }}</span></div>
          </div>
    ...
    <div v-if="activeTab === 'config'" class="stat-card space-y-5">
      <h3 class="text-sm font-semibold text-white">Configurazione Scanner 15m</h3>
...
import { useInst15mStore } from '@/stores/inst-scanner-15m'
...
const draft = ref({ minScore: 50, atrSlMult: 0.5, tpRr: 1.5, maxConcurrent: 3, autoEnter: true, liveEnabled: false })
watch(analytics, (a) => {
  if (!a?.config) return
